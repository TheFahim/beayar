...
<div id="revisionHistoryPanel"
    ...
    <div
        ...
        <button id="closeRevisionPanel"
            ...
            title="Close Panel">
            <svg class="h-5 w-5 text-gray-500 dark:text-gray-400 group-hover:text-red-500 transition-colors" fill="none"
    ...
    @if ((isset($hasAnyChallan) && $hasAnyChallan) || (isset($hasAnyBill) && $hasAnyBill))
        <div
            class="px-4 py-2 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 border-b border-yellow-200 dark:border-yellow-800">
            Activation locked due to existing {{ (isset($hasAnyBill) && $hasAnyBill) ? 'bill' : 'challan' }} for this
            quotation.
        </div>
    @endif
                    ...
                    <div
                                        ...
                                        <div class="flex items-center gap-2 mb-1">
                                            @if ($revision->is_active)
                                                <span
                                                    ...
                                                    <span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1 animate-ping"></span>
                                                    Active
                                                </span>
                                            @endif
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="text-right">
                                        <p class="text-sm font-semibold text-gray-900 dark:text-white">
                                            {{ $revision->type == 'normal' ? ($companySettings['exchange_rate_currency'] ?? 'BDT') : $revision->currency }}
                        ...
                        <div id="revisionCardDetails{{ $index }}"
                            ...
                            <div class="p-4 space-y-3">
                                ...
                                @if ($loadRevision && $loadRevision->id !== $revision->id)
                                    <div class="pt-2">
                                        <a href="{{ route('tenant.quotations.edit', ['quotation' => $quotation->id, 'revision_id' => $revision->id]) }}"
                                ...
                                <div class="pt-2">
                                    <form method="POST"
                                        action="{{ route('tenant.quotations.revisions.destroy', ['quotation' => $quotation->id, 'revision' => $revision->id]) }}"
