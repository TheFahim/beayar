    ...
    <x-ui.card class="mx-auto">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-8 p-2 mb-4 gap-4">
            ...
            <div class="col-span-1 lg:col-span-6">
                <form method="GET" action="{{ route('tenant.products.index') }}" class="w-full">
                    <div class="relative">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-white">
                <thead class="text-xs text-gray-700 uppercase bg-gray-300 dark:bg-gray-500 dark:text-gray-400">
                    <tr class="dark:text-white">
                        ...
                        <th scope="col" class="px-3 py-3 w-32 text-center">
                            Actions
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($products as $product)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-all duration-200 hover:shadow-lg hover:scale-[1.01]">
                                ...
                                <div class="relative w-16 h-16 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 group">
                                    ...
                                    @else
                                        <div class="w-full h-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center">
                                            <x-ui.svg.image class="w-8 h-8 text-gray-400 dark:text-gray-500" />
                                        </div>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="max-w-sm max-h-32 overflow-y-auto custom-scrollbar">
                            ...
                            <td class="px-3 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('tenant.products.edit', $product->id) }}" 
                                    ...
                                    <form action="{{ route('tenant.products.destroy', $product->id) }}" method="POST" class="inline-block" id="delete-form-{{ $product->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                            class="delete-button p-2 rounded-full transition-colors {{ $product->is_deletable ? 'bg-red-100 text-red-600 hover:bg-red-200 hover:text-red-800' : 'bg-gray-200 text-gray-400 cursor-not-allowed opacity-60' }}"
                                            title="{{ $product->is_deletable ? 'Delete' : 'This product is used in quotations/challans and cannot be deleted' }}"
                                            {{ !$product->is_deletable ? 'disabled' : '' }}>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center justify-center">
                                    <x-ui.svg.book class="w-12 h-12 mb-3 opacity-50" />
