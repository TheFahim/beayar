<x-dashboard.layout.default title="Edit Bill">
    ...
    <div>
        ...
        <form class="space-y-3" action="{{ route('tenant.bills.update', $bill->id) }}" method="POST">
            @csrf
            @method('PUT')
        ...
        <form method="POST" action="{{ route('tenant.bills.destroy', $bill->id) }}" id="delete-form" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
