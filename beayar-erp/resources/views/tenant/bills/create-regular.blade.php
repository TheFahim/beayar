<x-dashboard.layout.default title="Create Regular Bill">
    ...
    <div class="sticky top-20 z-40 mb-4 animate-fade-in-up">
        <div
            ...
            <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                ...
                <button type="submit" form="regularBillForm" aria-label="Create regular bill"
    ...
    <div class="max-w-7xl mx-auto p-6" x-data="{
        ...
        init() {
            ...
            this.discountEditable = parseFloat(this.activeRevision?.discount_amount ?? 0);
            this.shippingEditable = parseFloat(this.activeRevision?.shipping ?? 0);
            this.discount = this.discountEditable;
            this.shipping = this.shippingEditable;
            this.discountApplied = parseFloat(this.activeRevision?.discount_percentage ?? 0) > 0;
            this.shippingApplied = parseFloat(this.activeRevision?.shipping ?? 0) > 0;
        ...
        buildFromSelections() {
            ...
            const revDiscPct = parseFloat(this.activeRevision?.discount_percentage ?? 0);
            this.discountEditable = this.discountApplied ? (subtotal * revDiscPct / 100) : 0;
            ...
            const base = parseFloat(subtotal) +
                (this.shippingApplied ? (parseFloat(this.shippingEditable) || 0) : 0) -
                (this.discountApplied ? (parseFloat(this.discountEditable) || 0) : 0) +
                (this.vatApplied ? (parseFloat(this.vatEditable) || 0) : 0) +
                (parseFloat(this.roundUp) || 0);
            this.total = base.toFixed(2);
        },
        // Client-side validation mapping to StoreRegularBillRequest
        clientValidateAndSubmit(e) {
            ...
            this.$nextTick(() => document.getElementById('regularBillForm').submit());
        }
    }">
        <!-- Client-side validation is added to enforce required fields and formats per StoreRegularBillRequest -->
        <form id="regularBillForm" action="{{ route('tenant.bills.store') }}" method="POST"
            ...
            <div class="grid lg:grid-cols-12 gap-4">
                                ...
                                <div class="mt-1 space-y-1 max-h-32 overflow-y-auto custom-scrollbar pr-1">
                                    ...
                                    @forelse ($prevBills as $b)
                                        <div
                                            ...
                                            <span>BDT {{ number_format($b->total_amount ?? 0, 2) }}</span>
                ...
                <div class="lg:col-span-8 space-y-4">
                                                            ...
                                                            <tr class="border-t border-gray-100 dark:border-gray-700">
                                                                ...
                                                                <td class="text-center py-1">
                                                                    <input type="number" step="0.01"
                                                                        ...
                                                                        :disabled="{{ $p->remaining_quantity ?? $p->quantity }} <=
                                                                            0"
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <template
                                                                            x-if="calcStatus[{{ $p->id }}] === 'loading'">
                                                                            <span
                                                                                class="inline-flex items-center text-indigo-600">
                                                                ...
                                                                <td class="text-center py-1">
                                                                    <span
                                                                        x-text="(((selectedQuantity[{{ $p->id }}]||0) * {{ $qp->unit_price ?? 0 }}).toFixed(2))"></span>
                                                                </td>
                                                                <td class="text-center py-1">
                        ...
                        <div
                                ...
                                <div class="flex items-center gap-2 text-gray-900 dark:text-white">
                                    <div
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2" />
                                        </svg>
                                    </div>
                            ...
                            <div class="p-4">
                                <div class="grid md:grid-cols-4 gap-4">
                                    <div
                                        class="rounded-lg h-full border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-700/30 text-center">
                                        <div class="text-xs text-gray-700 dark:text-gray-200">Subtotal</div>
                                        <div class="text-2xl font-bold text-gray-900 dark:text-white"
                                            x-text="subtotal"></div>
                                    </div>
                                    <div
                                        class="rounded-lg h-full border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-700/30">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span
                                                ...
                                                x-text="discountApplied ? 'Applied' : 'Not Applied'"></span>
                                        </div>
                                        <div class="mt-2 text-center">
                                            <div class="text-xl font-bold text-gray-900 dark:text-white"
                                                x-text="parseFloat(discountEditable).toFixed(2)"></div>
                                            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">Percentage:
                                                <span
                                                    x-text="(activeRevision?.discount_percentage || 0) + '%' "></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span
                                                :class="vatApplied ?
                                                    ...
                                                    'bg-gray-100 text-gray-700 dark:bg-gray-700/50 dark:text-gray-300'"
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold whitespace-nowrap"
                                                x-text="vatApplied ? 'Applied' : 'Not Applied'"></span>
                                        </div>
                                        <div class="mt-2 text-center">
                                            <div class="text-xl font-bold text-gray-900 dark:text-white"
                                                x-text="parseFloat(vatEditable).toFixed(2)"></div>
                                            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">Percentage:
                                                <span x-text="(activeRevision?.vat_percentage || 0) + '%' "></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span
                                                :class="shippingApplied ?
                                                    'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                                                    'bg-gray-100 text-gray-700 dark:bg-gray-700/50 dark:text-gray-300'"
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold whitespace-nowrap"
                                                x-text="shippingApplied ? 'Applied' : 'Not Applied'"></span>
                                        </div>
                                        <div class="mt-2 grid grid-cols-6 gap-2 items-center">
                                            ...
                                            <div class="col-span-1 flex items-center justify-center">
                                                <input type="checkbox" class="rounded" x-model="shippingApplied"
                                                    @change="buildFromSelections()" aria-label="Apply shipping">
                                            </div>
                                        </div>
                                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">Max: <span
                                                x-text="(activeRevision?.shipping || 0).toFixed ? activeRevision.shipping.toFixed(2) : (activeRevision?.shipping || 0)"></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-4 grid md:grid-cols-4 gap-4 items-stretch">
                                    <div
                                        class="rounded-lg border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-700/30">
                                    ...
                                    <div
                                        class="md:col-span-2 rounded-lg p-4 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white">
                                        <div class="text-xs opacity-90">Total</div>
                                        <div class="text-3xl font-extrabold" x-text="total"></div>
                                    </div>
                                    <div
                                        class="rounded-lg border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-700/30 text-center">
                                        <div class="text-xs text-gray-700 dark:text-gray-200">Items</div>
                                        <div class="text-xl font-semibold text-gray-900 dark:text-white"
                                            x-text="items.length"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <div class="p-4">
                            <div class="grid md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    <!-- Required: invoice_no (string, unique) -->
                                    <label
                                    ...
                                    <input name="invoice_no" x-ref="invoice_no" type="text"
                                        class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                                        placeholder="INV-001" value="{{ old('invoice_no', $nextInvoiceNo) }}" required>
                                    @error('invoice_no')
                                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    <!-- Required: bill_date with format d/m/Y -->
                                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1.5">Bill
                                    ...
                                    <input name="bill_date" id="bill_date" x-ref="bill_date" type="text"
                                        ...
                                        placeholder="dd/mm/yyyy" inputmode="numeric" pattern="^\d{2}\/\d{2}\/\d{4}$" value="{{ date('d/m/Y') }}"
                                        required>
                                    @error('bill_date')
                                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    <!-- Optional: payment_received_date with format d/m/Y when provided -->
                                    <label
                                        class="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-1.5">Payment
                                        Received Date</label>
                                    <input name="payment_received_date" id="payment_received_date"
                                        x-ref="payment_received_date" type="text"
                                        class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 flowbite-datepicker"
                                        placeholder="dd/mm/yyyy" inputmode="numeric" pattern="^\d{2}\/\d{2}\/\d{4}$">
                            ...
                            <div class="grid md:grid-cols-1 gap-4 mb-4">
                                <div>
                                    <!-- Optional: notes (string, max 1000) -->
                                    <label
                    ...
                    <template x-for="(item, idx) in items" :key="idx">
                        <div>
                            <input type="hidden" :name="'items[' + idx + '][quotation_product_id]'"
                                :value="item.quotation_product_id">
                            <input type="hidden" :name="'items[' + idx + '][quantity]'" :value="item.quantity">
                            <template x-for="(alloc, aidx) in item.allocations" :key="aidx">
                                <div>
                                    <input type="hidden"
                                        :name="'items[' + idx + '][allocations][' + aidx + '][challan_product_id]'"
                                        :value="alloc.challan_product_id">
                                    <input type="hidden"
                                        :name="'items[' + idx + '][allocations][' + aidx + '][billed_quantity]'"
                                        :value="alloc.billed_quantity">
                                </div>
                            </template>
                        </div>
                    </template>
        ...
        <div x-show="clientSideErrors && clientSideErrors.length" class="mt-4 animate-fade-in-up">
            <div
                class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-4">
                    <ul class="space-y-2">
                        <template x-for="(err, idx) in clientSideErrors" :key="idx">
                            <li
                                class="flex items-start gap-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                                <svg class="w-4 h-4 text-yellow-600 dark:text-yellow-400 flex-shrink-0" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <p class="text-sm font-medium text-yellow-700 dark:text-yellow-300" x-text="err">
                                </p>
                            </li>
                        </template>
                    </ul>
