            ...
            <div class="px-4 py-3">
                ...
                <div class="flex items-center justify-between gap-4 text-sm">
                    <div class="flex items-start space-x-3">
                        ...
                        <span class="text-gray-900 dark:text-gray-100 break-words">{{ $quotation->customer->company->name ?? $quotation->customer->company_name }}</span>
                    </div>
                    <div class="flex items-start space-x-3">
                        <span class="font-medium text-gray-600 dark:text-gray-300 whitespace-nowrap">Address:</span>
                        <span class="text-gray-900 dark:text-gray-100 break-words">{{ $quotation->customer->company->address ?? $quotation->customer->address }}</span>
                    </div>
                </div>
            </div>
        </div>

        <form class="space-y-4" action="{{ route('tenant.challans.store') }}" method="POST" enctype="multipart/form-data" @submit.prevent="validateForm() ? $el.submit() : null">
            @csrf
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
                            <svg class="w-5 h-5 mr-2 text-olive-600 dark:text-olive-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
