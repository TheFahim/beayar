<x-dashboard.layout.default title="New Payments">
    <x-dashboard.ui.bread-crumb>
        <li class="inline-flex items-center">
            <a href="{{ route('tenant.received-bills.index') }}"
    ...
    <x-ui.card heading="Add New Payments">
        <form action="{{ route('tenant.received-bills.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-2 mx-5">
                <div class="grid grid-cols-3">
                    <div>
                        <x-ui.form.searchable-select name="bill_id" x-model="bill_id"
                            api-endpoint="{{ route('tenant.bills.search') }}" />
                    </div>
