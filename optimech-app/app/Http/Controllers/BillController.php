...
class BillController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return view('dashboard.bills.index', compact('bills', 'latestByQuotation', 'metrics'));
        } catch (\Throwable $e) {
    ...
    public function createAdvanceView(Quotation $quotation, $activeRevision, $existingAdvanceBill = null)
    {
        ...
        return view('dashboard.bills.create-advance', compact('quotation', 'activeRevision', 'existingAdvanceBill', 'nextInvoiceNo'));
    }
    ...
    public function createRegularView(Quotation $quotation, $activeRevision, $challans)
    {
        ...
        $nextInvoiceNo = $this->invoiceNumberGenerator->generate($quotation);
        return view('dashboard.bills.create-regular', compact('quotation', 'activeRevision', 'challans', 'nextInvoiceNo'));
    }
    ...
    public function createRunningView(Quotation $quotation, Bill $parentBill)
    {
        ...
        $nextInvoiceNo = $this->invoiceNumberGenerator->generate($quotation);
        return view('dashboard.bills.create-running', compact('quotation', 'parentBill', 'nextInvoiceNo'));
    }
    ...
    public function show(Bill $bill)
    {
        ...
        if (! Auth::user()->hasRole('admin') && $bill->quotationRevision->created_by != Auth::id()) {
        ...
        return view('dashboard.bills.show', compact('bill', 'subtotal', 'histories'));
    }
    ...
    public function editAdvance(Bill $bill)
    {
        ...
        return view('dashboard.bills.edit-advance', compact('bill', 'quotation', 'activeRevision'));
    }
    ...
    public function updateAdvance(UpdateAdvanceBillRequest $request, Bill $bill)
    {
        ...
        // return redirect()->route('bills.show', $bill)->with('success', 'Advance bill updated successfully.');
    }
    ...
    public function destroy(Bill $bill)
    {
        ...
        // if ($bill->receivedBills()->exists()) {
        //     Log::warning('Blocked bill deletion: received payments exist', [
        //         'bill_id' => $bill->id,
        //         'quotation_id' => $bill->quotation_id,
        //     ]);
        //     abort(403, 'Cannot delete a bill that has received payments');
        // }
    ...
    public function createFromQuotation(Quotation $quotation)
    {
        ...
        if ($advanceBills->isNotEmpty()) {
            ...
            return view('dashboard.bills.running', compact('quotation', 'advanceBills'));
        }

        if ($challans->isNotEmpty()) {
            $this->calculateRemainingQuantitiesForChallanProducts($challans);

            return view('dashboard.bills.regular', compact('quotation', 'activeRevision', 'challans'));
        }

        return view('dashboard.bills.index', compact('quotation', 'activeRevision'));
    }
    ...
    public function editRunning(Bill $bill)
    {
        ...
        return view('dashboard.bills.edit-running', compact('bill', 'quotation', 'parentBill'));
    }
    ...
    public function updateRunning(Request $request, Bill $bill)
    {
        ...
        try {
            ...
            return redirect()->route('bills.index')->with('success', 'Running bill updated successfully.');
        } catch (ValidationException $e) {
            return redirect()->back()->withInput()->withErrors($e->errors());
        } catch (\Exception $e) {
    ...
    public function editRegular(Bill $bill)
    {
        ...
        return view('dashboard.bills.edit-regular', compact('bill', 'quotation', 'activeRevision', 'challans'));
    }
