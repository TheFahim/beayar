...
class ChallanController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $quotation->customer->load('company');

        $revision = QuotationRevision::with(['products', 'quotation.customer'])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('challans.show', $challan->id)
    ...
    public function show(Challan $challan)
    {
        ...
        // $ownerId = $challan->revision->created_by ?? null;
        // if (! Auth::user()->hasRole('admin') && ($ownerId !== null) && ($ownerId != Auth::id())) {
        ...
        return view('dashboard.challans.show', compact('challan', 'hasBill', 'latestBill'));
    }
    ...
    {
        $challan->load(['revision.quotation.customer', 'products.quotationProduct.product']);
    ...
    public function update(Request $request, Challan $challan)
    {
        ...
        $validator = Validator::make($request->all(), [
            'quotation_revision_id' => 'required|exists:quotation_revisions,id',
            'date' => 'required|date_format:d/m/Y',
            'challan_no' => ['required', 'string', 'max:255', Rule::unique('challans', 'challan_no')->ignore($challan->id)],
            'po_no' => [
                'required',
                'string',
                'max:255',
    ...
    public function destroy(Challan $challan)
    {
        ...
        try {
            ...
            return redirect()->route('challans.index')->with('error', 'Failed to delete challan: '.$e->getMessage());
        }
    }
