    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($data['bill_type'] === 'running') {
                ...
                $remainingAmount = $parentBill->getRemainingAmount();
                if (($data['installment_amount'] ?? 0) > $remainingAmount) {
                    $validator->errors()->add('installment_amount', "Installment amount exceeds remaining balance ({$remainingAmount})");
                }

                $remainingPercentage = $parentBill->getRemainingPercentage();
                if (($data['installment_percentage'] ?? 0) > $remainingPercentage) {
                    $validator->errors()->add('installment_percentage', "Installment percentage exceeds remaining balance ({$remainingPercentage}%)");
                }
            }
            ...
            if ($data['bill_type'] === 'advance') {
                if (empty($data['quotation_revision_id'])) {
                    $validator->errors()->add('quotation_revision_id', 'Quotation revision is required for advance bills');
                }
            }

            // Validate regular bill constraints
            if ($data['bill_type'] === 'regular') {
                if (empty($data['challan_ids'])) {
                    $validator->errors()->add('challan_ids', 'At least one challan must be selected for regular bills');
                }
            }
        });
    }
