            ...
            <div class="px-4 py-3">
                ...
                <div class="flex items-center justify-between gap-4 text-sm">
                    <div class="flex items-start space-x-3">
                        ...
                        <span class="text-gray-900 dark:text-gray-100 break-words">{{ $challan->revision->quotation->customer->company_name }}</span>
                    </div>
                    <div class="flex items-start space-x-3">
                        <span class="font-medium text-gray-600 dark:text-gray-300 whitespace-nowrap">Address:</span>
                        <span class="text-gray-900 dark:text-gray-100 break-words">{{ $challan->revision->quotation->customer->address }}</span>
                    </div>
                </div>
            </div>
        </div>

        <form class="space-y-4" action="{{ route('challans.update', $challan->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="validateForm() ? $el.submit() : null">
            @csrf
            @method('PUT')
