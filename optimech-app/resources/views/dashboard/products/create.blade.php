<x-dashboard.layout.default title="Create Products">
    ...
    {{-- Main Alpine component to manage an array of products --}}
    <div x-data="productsForm()" x-init="init()" class="space-y-6">
        <form method="POST" action="{{ route('products.store') }}" class="space-y-8" novalidate
            ...
            <template x-for="(product, index) in products" :key="product.key">
                <div x-data="{ isHovered: false }" @mouseenter="isHovered = true" @mouseleave="isHovered = false"
                    ...
                    {{-- Header for each product card --}}
                    {{-- Header for each product card --}}
                    <div class="flex items-center justify-between relative z-10">
                        <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center gap-3">
                            {{-- CORRECTED H3 STRUCTURE --}}
                            <span>Product #</span>
                            <span
                                class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-white text-sm font-bold flex items-center justify-center shadow-lg transition-transform duration-300"
                                :class="{ 'scale-110 rotate-12': isHovered }" x-text="index + 1"></span>
                        </h3>
                        <button type="button" @click="removeProduct(index)" x-show="products.length > 1"
                            class="group relative px-4 py-2 text-red-600 hover:text-white font-medium text-sm rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95">
                            <span
                                class="absolute inset-0 w-full h-full bg-gradient-to-r from-red-500 to-red-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
                    ...
                    {{-- Upload Modal --}}
                    {{-- MOVED: Upload Modal is now outside the loop to prevent duplication and nested forms --}}
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                        <template x-for="(spec, specIdx) in product.specifications"
                                :key="spec.key">
                            <div x-data="{ specHovered: false }" @mouseenter="specHovered = true"
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <button type="button" @click.stop="removeSpec(index, specIdx)"
                                ...
                                <div class="flex-1 relative min-w-0">
                                    <textarea :id="`text-area-${index}-${specIdx}`" :name="`products[${index}][specifications][${specIdx}][description]`"
                                        x-model="spec.description" required rows="4"
                        ...
                        <div class="mt-4">
                            <button type="button" @click="addSpec(index)"
                        ...
                        @if ($errors->has('products.*.specifications') || $errors->has('products.*.specifications.*.description'))
                            <div
            ...
            {{-- Global Actions --}}
            <div
                class="flex items-center justify-between mt-8 p-6 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-800 dark:to-gray-800 rounded-2xl border-2 border-gray-200 dark:border-gray-700 shadow-lg">
                <button type="button" @click="addProduct()"
                    class="group relative px-6 py-3 bg-gradient-to-r from-gray-700 to-gray-800 dark:from-gray-600 dark:to-gray-700 text-white rounded-xl hover:from-gray-800 hover:to-gray-900 dark:hover:from-gray-500 dark:hover:to-gray-600 font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden">
                    <span
                        class="absolute inset-0 w-full h-full bg-white opacity-0 group-hover:opacity-10 transition-opacity duration-300"></span>
                    <span class="relative flex items-center gap-2">
    ...
    <div x-show="showUploadModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="showUploadModal" x-transition:enter="ease-out duration-300"
                ...
                <form @submit.prevent="uploadImageForProduct()">
                    <div
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="modal-image-name" x-model="modalImageName"
                                required
                        ...
                        <x-ui.form.image-upload title="Product Images" name="modal_image"
                            id="modal-image-upload" :required="true" />
                    </div>
    ...
    {{-- Alpine components scripts --}}
    <script>
        function productsForm() {
            return {
                // Load products from old input if validation fails, otherwise start with an empty array.
                ...
                // Method to generate a new, empty product object.
                newProductSchema() {
                    return {
                        key: Date.now() + Math.floor(Math.random() * 1000000), // Unique key for Alpine's x-for
                        name: '',
                        imageId: null,
                        imageUrl: null,
                        specifications: [{
                ...
                init() {
                    // If the products array is empty after initialization (e.g., on a fresh page load),
                    // add one product form so the user has something to start with.
                ...
                validateForm(event) {
                    ...
                    if (!isValid) {
                        ...
                        if (firstErrorId) {
                            ...
                            if (element) {
                                // The textarea is hidden, scroll to the sun-editor-container instead if possible
                                ...
                                if (editorContainer && editorContainer.classList.contains('sun-editor')) {
                                    ...
                                    // Add a visual shake or border effect if possible (optional)
                                } else {
                ...
                addProduct() {
                    // Add a new product object to the array using the internal schema method.
                    this.products.push(this.newProductSchema());
                },
                ...
                addSpec(productIndex) {
                    this.products[productIndex].specifications.push({
                        key: Date.now() + Math.floor(Math.random() * 1000000),
                        description: ''
                    });
                },

                removeSpec(productIndex, specIndex) {
                    if (this.products[productIndex].specifications.length > 1) {
                        this.products[productIndex].specifications.splice(specIndex, 1);
                    }
                },

                openImageLibrary(productIndex) {
                    window.__selectImageCallback = (image) => {
                        this.products[productIndex].imageId = image.id;
                        this.products[productIndex].imageUrl = image.path;
                ...
                clearSelectedImage(productIndex) {
                    this.products[productIndex].imageId = null;
                ...
                async uploadImageForProduct() {
                    ...
                    try {
                        ...
                        if (response.ok && data.success) {
                            // Assign the uploaded image to the current product
                            this.products[this.currentProductIndex].imageId = data.image.id;
                            this.products[this.currentProductIndex].imageUrl = data.image.path.startsWith('http') ?
                                data.image.path :
                ...
                openUploadModal(index) {
                    // 1. Trigger global cleanup (defined in image-scripts)
                    window.dispatchEvent(new CustomEvent('trigger-upload-modal'));

                    if (typeof window.cleanupUploadModal === 'function') {
                ...
                clearModalState() {
                    this.modalImageName = '';
                    // Redundant safety clear for local DOM
                    const fileInput = document.getElementById('modal-image-upload');
    ...
    @include('dashboard.products.partials.image-scripts')
