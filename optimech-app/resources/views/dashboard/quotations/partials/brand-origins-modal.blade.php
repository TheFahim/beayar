    ...
    document.addEventListener('alpine:init', () => {
            ...
            async submitBrandOrigin() {
                ...
                try {
                    const url = this.isEditing ?
                        `/dashboard/brand-origins/${this.editingBrandOriginId}` :
                        '{{ route('brand-origins.store') }}';
        ...
        Alpine.data('brandOriginSearchableSelect', (config) => ({
            ...
            async generateCustomerNumber(company) {
                try {
                    const response = await fetch(
                        `/dashboard/companies/${company.id}/next-customer-serial`);
                    const data = await response.json();
            ...
            async deleteBrandOrigin(brandOrigin) {
                ...
                try {
                    const response = await fetch(`/dashboard/brand-origins/${brandOrigin.id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector(
