    ...
    <div class="mx-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="mx-2 lg:col-span-1">
            ...
            <x-ui.form.searchable-select name="quotation[customer_id]" x-model="selectedCustomerId"
                apiEndpoint="{{ route('customers.search') }}"
                perPage="20"
                displayTemplate="{customer_no} - {customer_name} ({company.name})"
                :searchFields="['customer_name', 'customer_no', 'phone', 'email', 'company.name', 'attention']"
            ...
            <div x-show="selectedCustomer" class="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 dark:text-white rounded text-sm">
                <span x-text="selectedCustomer ? `Selected: ${selectedCustomer.customer_name}${selectedCustomer.attention ? ' (Attn: ' + selectedCustomer.attention + ')' : ''}` : ''"></span>
            </div>
        </div>
