<x-dashboard.layout.default title="Quotations">
    ...
    <div class="max-w-full" x-data="quotationsPageData('{{ route('quotations.index') }}', '{{ request('date_from') }}', '{{ request('date_to') }}')" x-init="initDateRangePicker()">
        ...
        <x-ui.card>
            ...
            <div x-show="filtersOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <form method="GET" action="{{ route('quotations.index') }}" x-ref="filterForm"
        ...
        {{--
        <!-- Alpine.js Alert Notification -->
        <div x-show="showAlert" x-transition:enter="transition ease-out duration-300 transform"
            x-transition:enter-start="translate-x-full opacity-0" x-transition:enter-end="translate-x-0 opacity-100"
            x-transition:leave="transition ease-in duration-200 transform"
            x-transition:leave-start="translate-x-0 opacity-100" x-transition:leave-end="translate-x-full opacity-0"
            class="fixed top-4 right-4 z-50 px-4 py-3 rounded border shadow-lg"
            :class="alertType === 'success' ?
                'bg-green-100 dark:bg-green-900/30 border-green-400 dark:border-green-600 text-green-700 dark:text-green-200' :
                'bg-red-100 dark:bg-red-900/30 border-red-400 dark:border-red-600 text-red-700 dark:text-red-200'">
            <div class="flex items-center">
                <i class="fas mr-2"
                    :class="alertType === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'"></i>
                <span x-text="alertMessage"></span>
                <button class="ml-4 text-current hover:opacity-75" @click="hideAlert()">
