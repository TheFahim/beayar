<x-dashboard.layout.default title="Edit Running Bill">
    <x-dashboard.ui.bread-crumb>
        ...
        <x-dashboard.ui.bread-crumb-list name="Edit Running Bill" />
    </x-dashboard.ui.bread-crumb>
    ...
    <div class="sticky top-20 z-40 mb-4 animate-fade-in-up">
        <div
            ...
            <div class="flex-1 min-w-0">
                <h1 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-0.5">Edit Running Bill</h1>
                <p class="text-xs text-gray-600 dark:text-gray-400">Update running payment against an advance bill</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                ...
                <button type="submit" form="editRunningBillForm"
                    ...
                    <span class="text-sm font-semibold">Update Running Bill</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <form id="editRunningBillForm" class="space-y-6" action="{{ route('bills.update-running', $bill) }}" method="POST"
        x-data="{
            ...
            init() {
                ...
                const fallbackDue = Math.max(0, this.total - childrenSum);
                const preDue = parseFloat(this.bill?.due ?? 0);
                const curAmt = parseFloat(this.bill?.bill_amount ?? 0);
                const preCurrentDue = (isNaN(preDue) ? 0 : preDue) + (isNaN(curAmt) ? 0 : curAmt);
                this.dueCurrent = preCurrentDue > 0 ? preCurrentDue : parseFloat(fallbackDue);
                this.runAmt = parseFloat(this.bill?.bill_amount || 0);
                this.runPct = parseFloat(this.bill?.bill_percentage || 0);
            ...
            calculateBillPct() {
                let value = parseFloat(this.parentBill?.bill_percentage || 0);
                const children = Array.isArray(this.parentBill?.children) ? this.parentBill.children : [];
                for (let i = 0; i < children.length; i++) {
                    const child = children[i];
                    if ((child?.id ?? null) !== (this.bill?.id ?? null)) {
                        value += parseFloat(child?.bill_percentage || 0);
                    }
                }
            ...
            setErrors() {
                const maxPct = Math.min(this.remainPct, 100);
                const maxAmt = parseFloat(this.dueCurrent || 0);
                this.errors.percentage = (parseFloat(this.runPct || 0) > maxPct) ? `Percentage cannot exceed ${maxPct}%` : '';
                this.errors.amount = (parseFloat(this.runAmt || 0) > maxAmt) ? `Amount cannot exceed {{ ($bill->quotationRevision?->type === 'via') ? ($bill->quotationRevision?->currency) : 'BDT' }} ${maxAmt.toFixed ? maxAmt.toFixed(2) : maxAmt}` : '';
            },
        ...
        <input type="hidden" name="quotation_revision_id" :value="bill?.quotation_revision_id || parentBill?.quotation_revision_id || ''">
                                        ...
                                        <div class="space-y-1 max-h-44 overflow-y-auto">
                                            @foreach (($quotation->bills ?? collect())->sortBy('bill_date') as $b)
                                                <div class="grid grid-cols-3 gap-2 text-xs items-center">
                                                    <span class=" text-gray-500 dark:text-gray-400 whitespace-nowrap">{{ Carbon\Carbon::parse($b->bill_date)->format('d/m/Y') }}</span>
                                                    <span class="font-bold text-gray-900 dark:text-white whitespace-nowrap text-right">{{ ($parentBill->quotationRevision?->type === 'via') ? ($parentBill->quotationRevision?->currency) : 'BDT' }}
                                                        {{ number_format($b->bill_amount ?? $b->total_amount, 2) }}</span>
                                                    <span class=" text-gray-600 dark:text-gray-400 whitespace-nowrap text-right">{{ number_format($b->bill_percentage ?? 0, 2) }}%</span>
                                                </div>
                                            @endforeach
                                        </div>
                            </div>
                            ...
                            <div class="mt-4 grid md:grid-cols-2 gap-4">
                                <div>
                                    <label
                                        ...
                                        Percentage</label>
                                    <div class="relative">
                                        <input name="bill_percentage_input" type="number" step="0.01"
                                            min="0" :max="remainPct"
                                ...
                                <div>
                                    <label
                                        ...
                                        Amount</label>
                                    <div class="relative">
                                        <input name="bill_amount_input" type="number" step="any"
                                            ...
                                            required>
                                        <span
                                            class="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-700 dark:text-blue-400 text-sm">{{ ($bill->quotationRevision?->type === 'via') ? ($bill->quotationRevision?->currency) : 'BDT' }}</span>
                                    </div>
                                    @error('bill_amount')
                                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="grid md:grid-cols-3 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input name="bill_date" id="bill_date" type="text"
                                        ...
                                        value="{{ \Carbon\Carbon::parse($bill->bill_date)->format('d/m/Y') }}" required>
                                    @error('bill_date')
                                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    ...
                                    <input name="payment_received_date" id="payment_received_date" type="text"
                                        class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 flowbite-datepicker"
                                        placeholder="dd/mm/yyyy"
                                        value="{{ $bill->payment_received_date ? \Carbon\Carbon::parse($bill->payment_received_date)->format('d/m/Y') : '' }}">
                            ...
                            <div class="grid md:grid-cols-1 gap-4 mb-4">
                                <div>
                                    ...
                                    <textarea name="notes" id="notes" rows="4" maxlength="500"
                                        class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 resize-none">{{ $bill->notes }}</textarea>
                                    @error('notes')
                                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
