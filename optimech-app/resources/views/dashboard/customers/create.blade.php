                        ...
                        <div class="space-y-4">
                            <div>
                                <x-ui.form.input name="customer_no" label="Customer Number" placeholder="Auto-generated"
                                    ...
                                    required value="{{ old('customer_no') }}" readonly />
                            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
            ...
            async submitCompany() {
                ...
                try {
                    const url = this.isEditing ?
                        `/dashboard/companies/${this.editingCompanyId}` :
                    ...
                    if (response.ok) {
                        if (this.isEditing) {
                            // Update the company in the searchable select
                            if (window.companySearchInstance) {
                                window.companySearchInstance.updateCompany(data.company);
                            }
                        } else {
                            // Add the new company to the searchable select
                            if (window.companySearchInstance) {
        ...
        Alpine.data('companySearchableSelect', (config) => ({
            ...
            checkSelection() {
                // Delay slightly to allow click event on option to register first
                setTimeout(() => {
                    if (!this.selectedValue) {
                        this.searchTerm = '';
                    } else {
                        // If value is selected but text doesn't match (user edited it), revert to selected name
                        const selected = this.allOptions.find(opt => opt.id == this.selectedValue);
                        if (selected) {
                            this.searchTerm = selected.name;
                        }
