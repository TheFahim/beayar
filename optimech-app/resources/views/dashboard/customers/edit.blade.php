    ...
    <div class="max-w-4xl mx-auto">
        <x-ui.card
            ...
            <form method="POST" action="{{ route('customers.update', $customer->id) }}">
                ...
                <!-- Header -->
                <div
                    class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-gray-800 dark:to-gray-900">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Update customer information and company details</p>
                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Customer number is auto-generated based on company code</p>
                            </div>
                            ...
                            <div class="flex items-end gap-4">
                                <div class="flex-1">
                                    ...
                                    <div x-data="companySearchableSelect({
                                        value: '{{ old('company_id', $customer->company_id) }}',
                                        endpoint: '{{ route('companies.search') }}'
                                    })" x-init="init" class="relative"
                                        @click.away="open = false">

                                        {{-- Hidden input for form submission --}}
                                        <input type="hidden" name="company_id" x-bind:value="selectedValue" required>
                                                            ...
                                                            <div class="flex items-center justify-between">
                                                                <div @click="selectOption(option)" class="flex-1">
                                                                    <div class="font-medium text-gray-900 dark:text-white"
                                                                        x-text="option.company_code ? `${option.name} (${option.company_code})` : option.name"></div>
                                                                    <div class="text-xs text-gray-500 dark:text-gray-400"
                                                                        x-text="option.address" x-show="option.address"></div>
                                                                </div>
                                                                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                                    <!-- Edit Button -->
                                                                    <button type="button" @click="editCompany(option)"
                                                                    ...
                                                                    <!-- Delete Button -->
                                                                    <button type="button" @click="deleteCompany(option)"
                ...
                <div class="h-20"></div>
                <div class="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-6 py-4">
                    <div class="flex justify-end space-x-4">
                        ...
                        <button type="submit"
                            class="px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-medium rounded-lg transition-all duration-300 hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl">
                            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<script>
    document.addEventListener('alpine:init', () => {
        // Company Modal Component with Edit Functionality
            ...
            async submitCompany() {
                ...
                try {
                    const url = this.isEditing ?
                        `/dashboard/companies/${this.companyForm.id}` :
                    ...
                    if (response.ok) {
                        if (window.companySearchInstance) {
                            if (this.isEditing) {
                                window.companySearchInstance.updateCompany(data.company);
                            } else {
                                window.companySearchInstance.addNewCompany(data.company);
                            }
                        }
                        this.closeModal();
                    ...
                    this.errors = {
                        general: 'An error occurred while processing the company.'
                    };
                } finally {
                    this.loading = false;
                }
        ...
        Alpine.data('companySearchableSelect', (config) => ({
            ...
            selectOption(option) {
                ...
                // Generate customer number when company is changed (only if current customer_no doesn't match the new company)
            ...
            async generateCustomerNumber(company) {
                try {
                    ...
                    if (!currentCustomerNo || !currentCustomerNo.startsWith(company.company_code + '-')) {
                        const response = await fetch(`/dashboard/companies/${company.id}/next-customer-serial`);
                        const data = await response.json();
            ...
            async deleteCompany(company) {
                ...
                try {
                    const response = await fetch(`/dashboard/companies/${company.id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            'Accept': 'application/json',
