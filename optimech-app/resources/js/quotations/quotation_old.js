...
function initQuotationForm(config = {}) {
    return {
        ...
        // ========================================================================
        // CURRENCY UTILITIES (Delegated to Helper)
        // ========================================================================

        getCurrencySymbol(currency = null) {
            return QuotationHelpers.getCurrencySymbol(currency || this.quotation_revision.currency);
        },

        formatCurrency(amount, currency = null) {
            return QuotationHelpers.formatCurrency(amount, currency || this.quotation_revision.currency);
        },

        // Display-only number formatter: 2 decimals, no symbol, no grouping
        format2(value) {
            const n = QuotationHelpers.parseFloat(value);
            return Number.isFinite(n) ? n.toFixed(2) : '0.00';
        },

        addVisualFeedback(elementId, cssClasses, duration = 2000) {
            try {
                const element = document.getElementById(elementId) ||
                    document.querySelector(`[data-feedback-id="${elementId}"]`);

                if (!element) {
                    console.warn(`Element with ID or data-feedback-id "${elementId}" not found for visual feedback`);
                    return;
                }

                const originalClasses = element.className;
                element.className = `${originalClasses} ${cssClasses}`;
        ...
        addFieldFeedback(fieldName, index, duration = 1000) {
            try {
                const selectors = [
                    `#${fieldName}_${index}`,
                    `[name="quotation_products[${index}][${fieldName}]"]`,
                    `[data-field="${fieldName}"][data-index="${index}"]`,
                    `input[name*="${fieldName}"][data-index="${index}"]`
                ];
                ...
                if (!element) {
                    console.warn(`Field element for "${fieldName}" at index ${index} not found for feedback`);
                    return;
                }

                const originalClasses = element.className;
                const feedbackClasses = 'bg-green-100 dark:bg-green-900/20 border-green-400 transition-colors duration-300';
                element.className = `${originalClasses} ${feedbackClasses}`;

                setTimeout(() => {
                    element.className = originalClasses;
                }, duration);
            } catch (error) {
                console.warn('Error applying field feedback:', error);
            }
        }
    };
}
