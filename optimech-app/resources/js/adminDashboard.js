...
document.addEventListener('alpine:init', () => {
    ...
    // User Quotation Statistics Component (Admin Only)
    Alpine.data('userQuotationStats', () => ({
        isDropdownOpen: false,
        isLoading: true,
        chart: null,
        users: [],
        ...
        async fetchData() {
            this.isLoading = true;
            try {
                const response = await fetch(`/dashboard/api/user-quotation-stats?filter=${this.selectedFilter.value}`);
                if (!response.ok) throw new Error('Network response was not ok');
                ...
                console.error('Error fetching user quotation stats:', error);
            } finally {
        ...
        updateChart() {
            const categories = this.users.map(u => u.user_name);
            const seriesData = this.users.map(u => u.quotation_count);
            ...
            const options = {
                ...
                tooltip: {
                    custom: ({ series, seriesIndex, dataPointIndex }) => {
                        const userData = this.users[dataPointIndex];
                        if (!userData) return '';
                        ...
                        return `
                            ...
                            <div style="padding: 8px; font-family: Inter, sans-serif; font-size: 12px;">
                                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                    ...
                                    <span style="font-weight: 600; margin-left: 15px;">${userData.quotation_count}</span>
                                </div>
                                <div style="display: flex; justify-content: space-between;">
                                    <span>Total Value:</span>
                                    <span style="font-weight: 600; margin-left: 15px;">৳${userData.total_value.toLocaleString()}</span>
                                </div>
                            </div>
                        `;
                    }
            ...
            if (this.chart) {
                ...
                this.chart = new ApexCharts(this.$refs.userQuotationChart, options);
                this.chart.render();
            }
        },
